...
class MainController extends Controller
{
    ...
    public function calculate(Request $req) {
        ...
        if ($req->tipe == 'SP') {
            ...
            try {
                $response = Http::post($apiUrl, ['bilangan' => $log->angka_input]);

                if ($response->successful()) {
                    $hasilKuadrat = $response->json('hasil_kuadrat');
                    $resultMessage = "Hasil akar kuadrat dari " . $log->angka_input . " adalah: " . $hasilKuadrat;
                    // Menyimpan nilai
                    $log->angka_output = $hasilKuadrat;
                } else {
